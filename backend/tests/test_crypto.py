...
@patch("src.auth.ticket.getenv", return_value=rsa_key)
def test_verify_ticket(_mock_getenv):
    ticket.get_private_key.cache_clear()
...
@patch("src.auth.ticket.getenv", return_value=rsa_key)
def test_get_public_key_pem(_mock_getenv):
    ticket.get_private_key.cache_clear()
